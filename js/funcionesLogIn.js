$(function () {
    ...
    (function (app) {
        ...
        app.agregarUsuario = function (tipo) {
            if (($('#nombreUsuario').val() != '') && ($('#apellidoUsuario').val() != '') && ($('#mailUsuario').val() != '') && ($('#celularUsuario').val() != '') && ($('#sexoUsuario').val() != '') && ($('#comprobarPass').val() != 0)) {
                var url = "controlador/ruteador/Ruteador.php?accion=agregar&Formulario=" + tipo + "&bool=hola";
                var datosEnviar = $("#form" + tipo).serialize() + '&pass='+btoa($.md5($('#password').val()));
                alert(datosEnviar);
                $.ajax({
                    url: url,
                    method: 'POST',
        ...
        app.bindings = function () {
            ...
            $("#loginUsuario").on('click', function () {
                var datos = 'u=' + btoa(btoa($('#txtUsuario').val())) + '&c=' + btoa($.md5($('#txtClave').val()));
